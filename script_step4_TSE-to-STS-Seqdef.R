...
tse_data <- data %>%
  pivot_longer(
    cols = matches("(poverty\\.bn|age)\\.w[1-9]$"), # Select the desired variables (poverty, age)
    names_to = c(".value", "wave"),                 # Split into value (base name) and wave
    names_pattern = "(.*)\\.w([1-9])"               # Extract base variable name and wave number
  ) %>%
  mutate(
    wave = as.integer(wave) # Convert wave to numeric for proper ordering
  ) %>%
  select(mergeid, wave, poverty = poverty.bn, age) # Keep ID, wave, and relevant variables
